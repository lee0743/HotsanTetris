                     ...
                     _In_ int       nCmdShow)
{
	...
	if (TRUE == blockBmpImage->Load24BitsBitmap("./assets/redBlock.bmp"))
	{
		...
		redBlockImage->Create(blockBmpImage->GetRawImage(), blockBmpImage->GetWidth(), blockBmpImage->GetHeight());
	}
	...
	for (int i = 0; i < MAX_BLOCK_UNIT_COUNT; ++i)
	{
		coord.LeftTop = { i * (int)(redBlockImage->GetWidth()), i * (int)(redBlockImage->GetHeight()) };
		coord.RightBottom = { coord.LeftTop.X + (int)(redBlockImage->GetWidth()), coord.LeftTop.Y - (int)(redBlockImage->GetHeight()) };

		block.AddBlockUnit(coord, redBlockImage);
	}
